...
const Publish = () => {
    ...
    const submitHandler = async (event: { preventDefault: () => void }) => {
        ...
        try {
            const { nft } = await metaplex.nfts().create({
                ...
                collection: new PublicKey(
                    "DYjyTe5ocgiH4HQgvdbodVmp6MD5AToyWA6u1dPYWJ2s"
                ),
        ...
        const journalistProgram = new anchor.Program<Journalist>(
            ...
            let mint_address = nft.address.toString();
            let timestampBN = new BN(timestamp);
            [authorAccount] = PublicKey.findProgramAddressSync(
                [wallet.publicKey.toBytes(), timestampBN.toArrayLike(Buffer, "be", 8)],
                new PublicKey(PROGRAM_ID)
            );
            const transaction = new Transaction();
            const ix = await journalistProgram.methods
                .upload(author, mint_address, timestampBN).accounts({initializer: wallet.publicKey, pdaAccount: authorAccount, systemProgram: SystemProgram.programId})
                .instruction();
            transaction.add(ix);
